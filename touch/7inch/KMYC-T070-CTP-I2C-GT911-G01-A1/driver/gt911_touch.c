...
esp_err_t kmyc_touch_start(void)
{
    ...
    ESP_RETURN_ON_FALSE(device_address != 0, ESP_ERR_NOT_FOUND, TAG,
                        "GT911 not found at 0x%02X or 0x%02X",
    ...
    ESP_RETURN_ON_ERROR(read_register(GT911_PRODUCT_ID_REG, identity, sizeof(identity)), TAG,
                        "failed to read GT911 identity");
    char product_id[5] = {0};
    for (size_t index = 0; index < sizeof(product_id) - 1; index++) {
        if (!isprint(identity[index])) {
            break;
        }
...
esp_err_t kmyc_touch_read(kmyc_touch_report_t *report, bool *updated)
{
    ...
    *updated = false;
    ...
    uint8_t status = 0;
    ESP_RETURN_ON_ERROR(read_register(GT911_REPORT_STATUS_REG, &status, 1), TAG,
    ...
    if (count > KMYC_TOUCH_MAX_POINTS) {
        ESP_RETURN_ON_ERROR(write_u8(GT911_REPORT_STATUS_REG, 0), TAG,
                            "failed to clear invalid touch report");
        return ESP_ERR_INVALID_RESPONSE;
    }

    uint8_t raw[KMYC_TOUCH_MAX_POINTS * GT911_POINT_BYTES] = {0};
    if (count > 0) {
        ESP_RETURN_ON_ERROR(read_register(GT911_FIRST_POINT_REG, raw,
                                          count * GT911_POINT_BYTES), TAG,
                            "failed to read touch points");
    }
    for (uint8_t index = 0; index < count; index++) {
        const uint8_t *point = &raw[index * GT911_POINT_BYTES];
